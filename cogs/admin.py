...
class AdminCog(commands.Cog):
    ...
    @admin_group.command(name="stats", description="View server engagement statistics & bot impact metrics (Admin only).")
    @is_admin()
    @app_commands.guild_only()
    async def admin_stats(self, interaction: discord.Interaction):
        stats = database.get_server_engagement_stats(interaction.guild_id)
        
        embed = utils.create_embed(
            title=f"📊 Server Engagement & Impact Analytics — {interaction.guild.name}",
            description="Detailed telemetry showcasing member activity, voice participation, racing events, and economy engagement generated by Discord Grand Prix on this server.",
            color=utils.COLOR_INFO
        )

        embed.add_field(
            name="👥 Server Community & Racers",
            value=(
                f"• **Registered Drivers:** `{stats['total_racers']:,}`\n"
                f"• **Active Members Today:** `{stats['active_today']:,}`\n"
                f"• **Total Economy Wealth:** `{stats['total_wealth']:,} credits`\n"
                f"• **Average Racer Level:** `Lvl {stats['avg_level']}`"
            ),
            inline=False
        )

        embed.add_field(
            name="💬 Community Chat & Voice Engagement",
            value=(
                f"• **Chat Credits Earned Today:** `{stats['chat_credits_today']:,} credits`\n"
                f"• **Voice Credits Earned Today:** `{stats['voice_credits_today']:,} credits`\n"
                f"• **Est. Voice Channel Time Today:** `~{stats['est_voice_minutes_today']:,} mins` (`{round(stats['est_voice_minutes_today']/60, 1)} hrs`)"
            ),
            inline=False
        )

        embed.add_field(
            name="🏁 Grand Prix & Race Events",
            value=(
                f"• **Grand Prix Events Scheduled:** `{stats['total_gps']:,}` (`{stats['completed_gps']:,}` finished)\n"
                f"• **Total Driver Grid Entries:** `{stats['total_entries']:,}`\n"
                f"• **Grand Prix Prize Pool Payouts:** `{stats['total_gp_payouts']:,} credits`"
            ),
            inline=False
        )

        embed.add_field(
            name="⚔️ Rivalries & WDC Championships",
            value=(
                f"• **Head-to-Head 1v1 Duels:** `{stats['total_duels']:,}`\n"
                f"• **WDC Championship Seasons:** `{stats['total_seasons']:,}` (`{stats['completed_seasons']:,}` completed)\n"
                f"• **Season Calendar Rounds Completed:** `{stats['completed_season_rounds']:,} rounds`"
            ),
            inline=False
        )

        embed.add_field(
            name="🎲 Spectator Betting & Economy Activity",
            value=(
                f"• **Total Bets Placed:** `{stats['total_bets']:,}`\n"
                f"• **Total Betting Wagered Volume:** `{stats['total_bet_volume']:,} credits`\n"
                f"• **Total Betting Winnings Paid:** `{stats['total_bet_payouts']:,} credits`"
            ),
            inline=False
        )
        ...
        embed.add_field(
            ...
            value=(
                f"• **Total Car Parts Collected:** `{stats['total_inventory_items']:,}`\n"
                f"• **Parts Actively Installed:** `{stats['equipped_items']:,}`"
            ),
            inline=False
        )
        ...
        embed.set_footer(text=f"Server ID: {interaction.guild_id} • Discord Grand Prix Admin Telemetry", icon_url=avatar_url)
